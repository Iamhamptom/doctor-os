...
export default function ScribePage() {
  ...
  const runPipeline = async (blobOrText: Blob | string) => {
    ...
    try {
      ...
      // Build doc directly from data (not state — state hasn't updated yet)
      const docFromData = buildDocFromData(data);
      setEditedDoc(docFromData);
      setEditedConfidence(data.validation?.overallScore || 50);
      setShowReview(true);
  ...
  const buildDocFromData = (r: any) => {
    ...
    return [
      ...
      `Specialty: ${r.specialty || "general_practice"}`,
      `Confidence: ${v.overallScore || 0}%`,
      `KB Score: ${kb.score || 0}%`,
      `Generated by: Visio AI Scribe`,
      `Chief Complaint: ${a.chiefComplaint}`,
      "", "ICD-10 CODES (KB-VERIFIED)",
      ...a.icd10Codes.map((c: any) => `  ${c.code}  ${c.description}  [${c.confidence}%]`),
      ...(kb.pmbCodes?.length ? [`  PMB conditions: ${kb.pmbCodes.join(", ")}`] : []),
      "", "SUBJECTIVE", a.soap.subjective || "—",
      "", "OBJECTIVE", a.soap.objective || "—",
      "", "ASSESSMENT", a.soap.assessment || "—",
      "", "PLAN", a.soap.plan || "—",
      ...(a.medications?.length ? ["", "MEDICATIONS (NAPPI-VERIFIED)", ...a.medications.map((m: any) => `  ${m.name} ${m.dosage} — ${m.frequency}`)] : []),
      ...(kb.suggestedTariffs?.length ? ["", "SUGGESTED TARIFF", `  ${kb.suggestedTariffs[0].code} (${kb.suggestedTariffs[0].description})`] : []),
      ...(a.redFlags?.length ? ["", "RED FLAGS", ...a.redFlags.map((f: string) => `  ! ${f}`)] : []),
      ...(kb.enrichments?.length ? ["", "KB ENRICHMENTS", ...kb.enrichments.map((e: string) => `  + ${e}`)] : []),
      ...(kb.issues?.length ? ["", "KB ISSUES", ...kb.issues.map((i: string) => `  ? ${i}`)] : []),
    ].join("\n");
  };

  const buildDoc = () => {
    if (!result) return "";
    return buildDocFromData(result);
  };
    ...
    <div className="p-4 lg:p-6 h-full flex flex-col">
      ...
      {showReview && result && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setShowReview(false)}>
          <div className="w-full max-w-4xl h-[90vh] rounded-lg ring-1 ring-border bg-card flex flex-col" onClick={e => e.stopPropagation()}>
